...
public class C01_SoftAssert {
    ...
    public void test01(){
        ...
        // amazon anasayfaya gittiginizi dogrulayin
                   //doğrulama=soft assert
        SoftAssert softAssert= new SoftAssert(); // hemen sayfa sonunda softAssert.assertAll() oluşturmalıyız
        String expectedUrlKelime="amazon";
        String actualUrl= Driver.getDriver().getCurrentUrl();
        softAssert.assertTrue(actualUrl.contains(expectedUrlKelime),"Url amazon icermiyor");
                                                     //mesaja failed olma durumunu yazarız
        // Nutella aratin
        ...
        javaSonucSayisiStr=javaSonucSayisiStr.replaceAll("\\W",""); // 6000
                //6,000 içerisindeki virgülü silmek için \\W kullandık, word olmayan herşeyi hiçlik yap
        ...
        softAssert.assertAll(); //soft assert oluşturduğumuz anda bunu da hemen oluşturmalıyız
         //yaptığımız 3 assertion'nın raporlamasını yapar

        Driver.closeDriver(); //assertAll failed olursa buraya ulaşmaz, sayfa kapanmaz

        //soft assert'te testlerimiz failed olsa da sorun değil
        //ancak hard assert olsaydı ilk failed olanda test dururdu, 2. ve 3.yü yapmaya izin vermezdi
    }
}
